#ifndef FREENOVE_RFID_LIB_FOR_PICO_h
#define FREENOVE_RFID_LIB_FOR_PICO_h

#include <Arduino.h>
#include <SPI.h>

#define MAX_LEN 16   // Maximum length of array
...
//Mifare_One¿¨Æ¬ÃüÁî×Ö
#define PICC_REQIDL           0x26               //The antenna area has not entered sleep mode
#define PICC_REQALL           0x52               //Find all cards within the antenna area
...
#define PICC_HALT             0x50               //hibernate
...
class RFID
{
  public:
	...
	unsigned char serNum[5];       // 4-byte card serial number, with the 5th byte being the checksum byte
	void init();
	void reset();
